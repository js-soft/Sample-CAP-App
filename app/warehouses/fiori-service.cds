...
annotate CatalogService.Warehouses with @(UI : {
  ...
  SelectionFields: [ name, city, email ],

  // columns in warehous list
  LineItem: [
    { Value: ID,       Label: '{i18n>ID}',      @UI.Importance: #High },
    { Value: name,     Label: '{i18n>Name}',    @UI.Importance: #High },
    { Value: address,  Label: '{i18n>Address}', @UI.Importance: #Medium },
    { Value: city,     Label: '{i18n>City}',    @UI.Importance: #Medium },
    { Value: email,    Label: '{i18n>Email}',   @UI.Importance: #Low }
  ]
});
...
annotate CatalogService.Warehouses with @(UI : {
  // page sections
  Facets: [
    { $Type : 'UI.ReferenceFacet', Label : '{i18n>Details}',   Target: '@UI.FieldGroup#WarehouseInfo' },
    // Pek til den ukvalifiserte LineItem-annotasjonen på navigasjonen:
    { $Type : 'UI.ReferenceFacet', Label : '{i18n>Inventory}', Target: 'stocks/@UI.LineItem' }
  ],
...
annotate CatalogService.Availabilities with @(UI : {
  LineItem: [
    { Value: book.title,     Label: '{i18n>Book}' },
    { Value: warehouse.name, Label: '{i18n>Warehouse}' },
